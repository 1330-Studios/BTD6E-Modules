...
public class DLLFromMemory : IDisposable {
    ...
    public TDelegate GetDelegateFromFuncName<TDelegate>(string funcName) where TDelegate : class => !typeof(Delegate).IsAssignableFrom(typeof(TDelegate))
            ? throw new ArgumentException(typeof(TDelegate).Name + " is not a delegate")
            : Marshal.GetDelegateForFunctionPointer(GetPtrFromFuncName(funcName), typeof(TDelegate)) is not TDelegate res ? throw new DllException("Unable to get managed delegate") : res;
    ...
    IntPtr GetPtrFromFuncName(string funcName) {
        ...
        if (string.IsNullOrEmpty(funcName)) throw new ArgumentException(null, nameof(funcName));
    ...
    public int MemoryCallEntryPoint() => Disposed
            ? throw new ObjectDisposedException("DLLFromMemory")
            : IsDll || _exeEntry == null || !_isRelocated
    ...
    void MemoryLoadLibrary(byte[] data) {
        ...
        checked {
            IntPtr oldHeader_OptionalHeader_ImageBase = Is64BitProcess
                ? (IntPtr)unchecked((long)OrgNTHeaders.OptionalHeader.ImageBaseLong)
                : unchecked((IntPtr)(OrgNTHeaders.OptionalHeader.ImageBaseLong >> 32));
            ...
            if (Is64BitProcess && PtrSpanBoundary(pCode, alignedImageSize, 32)) {
                ...
                var BlockedMemory = new List<IntPtr>();
                ...
                foreach (var ptr in BlockedMemory) _ = Win.VirtualFree(ptr, IntPtr.Zero, AllocationType.RELEASE);
                if (pCode == IntPtr.Zero) throw new DllException("Out of Memory");
            }

            // commit memory for headers
            var headers = Win.VirtualAlloc(pCode, (UIntPtr)OrgNTHeaders.OptionalHeader.SizeOfHeaders, AllocationType.COMMIT, MemoryProtection.READWRITE);
            ...
            if (locationDelta != IntPtr.Zero) {
                ...
                _ = Marshal.OffsetOf(typeof(IMAGE_NT_HEADERS), "OptionalHeader");
                _ = Marshal.OffsetOf(typeof(IMAGE_OPTIONAL_HEADER), "ImageBaseLong");
    ...
    static void CopySections(ref IMAGE_NT_HEADERS OrgNTHeaders, IntPtr pCode, IntPtr pNTHeaders, byte[] data) {
        ...
        for (var i = 0; i < OrgNTHeaders.FileHeader.NumberOfSections; i++, pSection = PtrAdd(pSection, Sz.IMAGE_SECTION_HEADER)) {
            ...
            if (Section.SizeOfRawData == 0) {
                ...
                if (size > 0) {
                    var dest = Win.VirtualAlloc(PtrAdd(pCode, Section.VirtualAddress), (UIntPtr)size, AllocationType.COMMIT, MemoryProtection.READWRITE);
                    ...
                    _ = Win.MemSet(dest, 0, (UIntPtr)size);
                }
                ...
                var dest = Win.VirtualAlloc(PtrAdd(pCode, Section.VirtualAddress), (UIntPtr)Section.SizeOfRawData, AllocationType.COMMIT, MemoryProtection.READWRITE);
    ...
    static bool PerformBaseRelocation(ref IMAGE_NT_HEADERS OrgNTHeaders, IntPtr pCode, IntPtr delta) {
        ...
        for (var pRelocation = PtrAdd(pCode, OrgNTHeaders.OptionalHeader.BaseRelocationTable.VirtualAddress); ;) {
            ...
            for (uint i = 0; i != RelCount; i++, pRelInfo = PtrAdd(pRelInfo, sizeof(ushort))) {
                ...
                switch ((BasedRelocationType)((ushort)Marshal.PtrToStructure(pRelInfo, typeof(ushort))! >> 12)!) {
                    case BasedRelocationType.IMAGE_REL_BASED_ABSOLUTE:
                        // skip relocation
                        break;
                    case BasedRelocationType.IMAGE_REL_BASED_HIGHLOW:
                        // change complete 32 bit address
                        var patchAddrHL = (int)Marshal.PtrToStructure(PtrAdd(pDest, (ushort)Marshal.PtrToStructure(pRelInfo, typeof(ushort))! & 0xfff), typeof(int))!;
                        patchAddrHL += (int)delta;
                        Marshal.StructureToPtr(patchAddrHL, PtrAdd(pDest, (ushort)Marshal.PtrToStructure(pRelInfo, typeof(ushort))! & 0xfff), false);
                        break;
                    case BasedRelocationType.IMAGE_REL_BASED_DIR64:
                        var patchAddr64 = (long)Marshal.PtrToStructure(PtrAdd(pDest, (ushort)Marshal.PtrToStructure(pRelInfo, typeof(ushort))! & 0xfff), typeof(long))!;
                        patchAddr64 += (long)delta;
                        Marshal.StructureToPtr(patchAddr64, PtrAdd(pDest, (ushort)Marshal.PtrToStructure(pRelInfo, typeof(ushort))! & 0xfff), false);
                        break;
                    case BasedRelocationType.IMAGE_REL_BASED_HIGH:
                    case BasedRelocationType.IMAGE_REL_BASED_LOW:
                    case BasedRelocationType.IMAGE_REL_BASED_HIGHADJ:
                    case BasedRelocationType.IMAGE_REL_BASED_MIPS_JMPADDR:
                    case BasedRelocationType.IMAGE_REL_BASED_MIPS_JMPADDR16:
                    default:
                        break;
                }
            }
    ...
    static IntPtr[] BuildImportTable(ref IMAGE_NT_HEADERS OrgNTHeaders, IntPtr pCode) {
        var ImportModules = new List<IntPtr>();
        ...
        for (uint i = 0; i != NumEntries; i++, pImportDesc = PtrAdd(pImportDesc, Sz.IMAGE_IMPORT_DESCRIPTOR)) {
            ...
            if (PtrIsInvalidHandle(handle)) {
                foreach (var m in ImportModules) _ = Win.FreeLibrary(m);
                ImportModules.Clear();
            ...
            for (var SzRef = IntPtr.Size; ; pThunkRef = PtrAdd(pThunkRef, SzRef), pFuncRef = PtrAdd(pFuncRef, SzRef)) {
                ...
                WriteFuncRef = Win.IMAGE_SNAP_BY_ORDINAL(ReadThunkRef)
                    ? Win.GetProcAddress(handle, Win.IMAGE_ORDINAL(ReadThunkRef))
                    : Win.GetProcAddress(handle, PtrAdd(PtrAdd(pCode, ReadThunkRef), Of.IMAGE_IMPORT_BY_NAME_Name));
    ...
    static void FinalizeSection(SectionFinalizeData SectionData, uint PageSize, uint SectionAlignment) {
        ...
        if ((SectionData.Characteristics & Win.IMAGE_SCN_MEM_DISCARDABLE) > 0) {
            ...
            if (SectionData.Address == SectionData.AlignedAddress &&
                ...
                _ = Win.VirtualFree(SectionData.Address, SectionData.Size, AllocationType.DECOMMIT);
            }
            return;
        }
    ...
    static uint GetMachineType() => IntPtr.Size == 8 ? Win.IMAGE_FILE_MACHINE_AMD64 : Win.IMAGE_FILE_MACHINE_I386;
    ...
    static IntPtr GetRealSectionSize(ref IMAGE_SECTION_HEADER Section, ref IMAGE_NT_HEADERS NTHeaders) {
        ...
        return IntPtr.Size == 8 ? (IntPtr)unchecked((long)size) : unchecked((IntPtr)size);
    }
    ...
    public void Dispose() {
        if (_initialized) {
            _ = (_dllEntry?.Invoke(pCode, DllReason.DLL_PROCESS_DETACH, IntPtr.Zero));
            _initialized = false;
        }

        if (ImportModules != null) {
            foreach (var m in ImportModules) if (!PtrIsInvalidHandle(m)) _ = Win.FreeLibrary(m);
            ImportModules = null;
        }

        if (pCode != IntPtr.Zero) {
            _ = Win.VirtualFree(pCode, IntPtr.Zero, AllocationType.RELEASE);
            pCode = IntPtr.Zero;
    ...
    static T PtrRead<T>(IntPtr ptr) where T : notnull => (T)Marshal.PtrToStructure(ptr, typeof(T))!;
    static void PtrWrite<T>(IntPtr ptr, T val) where T : notnull => Marshal.StructureToPtr(val, ptr, false);
    ...
    static IntPtr PtrBitOr(IntPtr p, UIntPtr v) => IntPtr.Size == 8 ? (IntPtr)unchecked((long)(unchecked((ulong)p.ToInt64()) | v.ToUInt64())) : unchecked((IntPtr)(int)(unchecked((uint)p.ToInt32()) | v.ToUInt32()));
    static IntPtr PtrAlignDown(IntPtr p, UIntPtr align) => (IntPtr)unchecked((long)(unchecked((ulong)p.ToInt64()) & ~(align.ToUInt64() - 1)));
    static bool PtrIsInvalidHandle(IntPtr h) {
        return h == IntPtr.Zero || h == (IntPtr.Size == 8 ? (IntPtr)(long)-1 : (IntPtr)(-1));
    }
    ...
    static T BytesReadStructAt<T>(byte[] buf, int offset) where T : notnull {
